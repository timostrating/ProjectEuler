 ...
 */
public class Problem054_PokerHands extends Problem {  // TODO: Problem054_PokerHands
    public static void main(String[] args) { new Problem054_PokerHands().test(); }
    public void start() {
        ...
        for (String[] hand : hands) {
            String[] player1Cards = new String[]{hand[0], hand[1], hand[2], hand[3], hand[4]};
            String[] player2Cards = new String[]{hand[5], hand[6], hand[7], hand[8], hand[9]};

            if (getCardsValue(player1Cards) > getCardsValue(player2Cards)) {
                ans++;
            }
        }
    ...
    public void test () {
        System.out.println( getCardsValue( new String[] {"5H", "5C", "6S", "7S", "2D",  "KC", "3S", "8S", "8D", "TD"} ));
    }

    private int getCardsValue(String[] cards) {
        int value = 0;

        // High Card
        for (String card : cards) {
            int newValue = getSingleCardValue(card);
            if (newValue > value)
                value = newValue;
        }

        if (is_one_pair(cards))         value += 100;
        if (is_two_pair(cards))         value += 1_000;
        if (is_three_of_a_Kind(cards))  value += 10_000;
        if (is_straight(cards))         value += 100_000;
        if (is_flush(cards))            value += 1_000_000;
        if (is_full_house(cards))       value += 10_000_000;
        if (is_four_of_a_kind(cards))   value += 100_000_000;
        if (is_straight_flush(cards))   value += 1_000_000_000;
        if (is_royal_flush(cards))      value += 2_000_000_000;

        return value;
    }


    private int getSingleCardValue(String card) {
