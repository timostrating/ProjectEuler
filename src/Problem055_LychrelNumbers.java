 ...
 */
public class Problem055_LychrelNumbers extends Problem {
    public static void main(String[] args) { new Problem055_LychrelNumbers().start(); }
    private void start() {
        int counter = 0;
    ...
    private boolean isLychrelNumber(int value) { return isLychrelNumber( new BigInteger(""+value), 1); }
    private boolean isLychrelNumber(BigInteger value, int iteration) {
        String reverseString = new StringBuffer(value.toString()).reverse().toString();
        BigInteger reverse = new BigInteger(reverseString);

        BigInteger newValue = value.add(reverse);
        ...
        return isLychrelNumber (newValue, ++iteration);
    }
}
...
fifty iterations, or, (ii) no one, with all the computing power that exists, has managed so far to map it to a palindrome.
